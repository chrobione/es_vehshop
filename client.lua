...
CreateThread(function()
    ...
    WarMenu.CreateMenu('vehshop', 'default', 'Vehicle Shop')
    WarMenu.SetMenuMaxOptionCountOnScreen('vehshop', 11)

    WarMenu.CreateSubMenu('confirmMenu', 'vehshop', 'Confirm')
    WarMenu.CreateSubMenu('closeMenu', 'vehshop', 'Vehicle Shop')
    
    WarMenu.SetSubTitle('vehshop', 'Categories')
    WarMenu.SetSubTitle('confirmMenu', 'Purchase confirmation')    
    ...
    if Vdist(playerCoords.x, playerCoords.y, playerCoords.z, -46.56327, -1097.382, 25.99875) <= 14.0 then
        if IsPedInAnyVehicle(GetPlayerPed(-1), true) == false then
            TriggerEvent("fs_freemode:displayHelp", "Press ~INPUT_CONTEXT~ to browse vehicles")
        end
        ...
        if WarMenu.IsMenuOpened('vehshop') then
            ...
            elseif WarMenu.IsMenuOpened('confirmMenu') then
                if WarMenu.Button('Buy', fakecar.costs .." $") then
                    Citizen.Trace('Verifying user money amount for ' ..fakecar.model)
                    TriggerServerEvent('CheckMoneyForVeh', fakecar.model, tonumber(fakecar.costs))
                    WarMenu.CloseMenu()
                    elseif WarMenu.MenuButton('No', 'vehshop') then
...
AddEventHandler('FinishMoneyCheckForVeh', function(moneyCheck)
  if moneyCheck == true then  
        local playerPed = GetPlayerPed(-1)z
...
Citizen.CreateThread(function()
    ...
    WarMenu.CreateMenu('test', 'default', 'Vehicle inventory')
    WarMenu.SetTitleColor('test', 255, 255, 255)
    WarMenu.SetTitleBackgroundColor('test', 255, 255, 255)
    WarMenu.SetSubTitle('test', 'Options')
    while true do
        if WarMenu.IsMenuOpened('test') then
            if WarMenu.ComboBox('Owned vehicles', carList, currentItemIndex, selectedItemIndex, function(currentIndex, selectedIndex)
